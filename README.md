...
<ul>
    <li style="margin-bottom: 10px;"><strong>Camada de Apresentação (os-frontend):</strong> Interface SPA desenvolvida em React, utilizando o construtor ultraveloz Vite. Entrega alta reatividade e disponibiliza um Proxy Reverso interno em seu arquivo de configuração para interceptar a rota de comunicação com a API, blindando a aplicação contra falhas e bloqueios de CORS (Cross-Origin Resource Sharing). A interface roda na porta pública <code>:5600</code>.</li>
    <li style="margin-bottom: 10px;"><strong>Camada de Negócio (os-backend):</strong> API RESTful construída com Python e framework FastAPI, operando sobre o servidor ASGI Uvicorn na porta <code>:8000</code>. O processamento ocorre de forma 100% assíncrona, tratando requisições concorrentes massivas de maneira não bloqueante.</li>
...
<p align="justify">
    O ecossistema consome chaves e configurações estruturadas em tempo de execução para garantir a segurança, portabilidade e modularidade dos dados sensíveis. Nenhuma senha deve ser exposta no código-fonte. Segue o mapeamento técnico da matriz de variáveis:
</p>
...
<table border="1" cellpadding="10" cellspacing="0" style="border-collapse: collapse; width: 100%; font-family: Arial, sans-serif; font-size: 14px; margin-top: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    <thead>
        <tr style="background-color: #0F4C81; color: #FFFFFF; text-align: left;">
            <th style="padding: 12px; border: 1px solid #CBD5E1; font-weight: bold; width: 25%;">Variável de Ambiente</th>
            <th style="padding: 12px; border: 1px solid #CBD5E1; font-weight: bold; width: 20%;">Módulo Consumidor</th>
            <th style="padding: 12px; border: 1px solid #CBD5E1; font-weight: bold; width: 55%;">Função Operacional e Descrição Técnica</th>
        </tr>
    </thead>
    <tbody>
        <tr style="background-color: #F8FAFC;">
            ...
            <td style="padding: 12px; border: 1px solid #CBD5E1; color: #334155; line-height: 1.5;">String de conexão assíncrona segura que injeta o driver especialista do Postgres e gerencia o pool de sessões sem risco de deadlocks: <code style="font-family: monospace;">postgresql+asyncpg://postgres:senha@database:5432/sistema_os_db</code>.</td>
        </tr>
        ...
        <tr style="background-color: #EEF2F7;">
            <td style="padding: 12px; border: 1px solid #CBD5E1; font-family: monospace; font-weight: bold; color: #0F4C81;"><code>SMTP_HOST</code><br/><code>SMTP_USER/PASS</code></td>
            <td style="padding: 12px; border: 1px solid #CBD5E1; font-family: monospace; color: #334155;">main.py / auth</td>
            <td style="padding: 12px; border: 1px solid #CBD5E1; color: #334155; line-height: 1.5;">Credenciais de rede e autenticação do barramento SMTP institucional (ex: UFAM). Utilizadas exclusivamente pelo Backend para despacho assíncrono de notificações de recusa e códigos OTP efêmeros.</td>
        </tr>
        <tr style="background-color: #F8FAFC;">
            <td style="padding: 12px; border: 1px solid #CBD5E1; font-family: monospace; font-weight: bold; color: #0F4C81;"><code>SEED_ADMIN_*</code><br/><code>SEED_TECNICO_*</code></td>
            <td style="padding: 12px; border: 1px solid #CBD5E1; font-family: monospace; color: #334155;">main.py (Lifespan)</td>
            <td style="padding: 12px; border: 1px solid #CBD5E1; color: #334155; line-height: 1.5;">Credenciais de <strong>Auto-Seed Seguras</strong>. Variáveis lidas exclusivamente no primeiro boot do servidor para instanciar e criptografar a conta Mestre da Gerência e Subgerência de forma automática, blindando o código-fonte contra vazamentos de senhas padrão.</td>
        </tr>
    </tbody>
</table>
...
<p align="justify">
    Siga à risca os passos ordenados abaixo para clonar, configurar as credenciais seguras, compilar a infraestrutura dockerizada e executar o ecossistema completo de maneira previsível e limpa:
</p>
...
<h3>Passo 2: Configuração de Credenciais Seguras (.env)</h3>
<p align="justify">Para garantir a segurança cibernética e a injeção do <em>Auto-Seed</em>, você deve definir as senhas de produção antes de ligar a orquestração. Acesse a pasta do backend, duplique o modelo de gabarito e preencha as suas chaves institucionais (Banco, SMTP e Senhas do Gerente):</p>
<pre><code>cd os-backend
cp .env.example .env

# Edite o arquivo .env gerado com suas credenciais reais.
# Após salvar, retorne à pasta raiz do projeto:
cd ..</code></pre>

<h3>Passo 3: Higienização de Portas e Volumes (Reset de Segurança)</h3>
<p>Para mitigar conflitos decorrentes de execuções anteriores, lixos persistidos em cache ou travamentos nas portas lógicas <code>:5432</code> (PostgreSQL), <code>:8000</code> (FastAPI) e <code>:5600</code> (React/Vite), limpe totalmente os contêineres e expurgue volumes órfãos utilizando:</p>
<pre><code>docker compose down -v</code></pre>

<h3>Passo 4: Inicialização da Infraestrutura Automatizada</h3>
<p>Inicie o processo de orquestração multi-container. Este comando criará a rede isolada <code>os_network</code>, montará os volumes persistentes, disparará o healthcheck do Postgres e compilará as imagens otimizadas:</p>
<pre><code>docker compose up --build -d</code></pre>

<h3>Passo 5: Acessar os Módulos do Sistema</h3>
<p>Assim que o barramento do Docker Compose reportar o status estável das aplicações, utilize o seu navegador web para operar os ambientes através das portas mapeadas:</p>
...
<ul style="margin-top: 10px;">
    <li style="margin-bottom: 5px;"><strong>Monitorar o fluxo de logs unificado em tempo real:</strong><br/><code style="background-color: #1F2937; color: #F8FAFC; padding: 2px 6px; border-radius: 4px;">docker compose logs -f</code></li>
    <li style="margin-bottom: 5px;"><strong>Inspecionar a integridade física de processos, mapeamento de portas e saúde (Healthcheck):</strong><br/><code style="background-color: #1F2937; color: #F8FAFC; padding: 2px 6px; border-radius: 4px;">docker compose ps</code></li>
    <li style="margin-bottom: 5px;"><strong>Encerrar os serviços graciosamente (Graceful Shutdown) preservando o estado dos dados em disco:</strong><br/><code style="background-color: #1F2937; color: #F8FAFC; padding: 2px 6px; border-radius: 4px;">docker compose down</code></li>
    <li><strong>Acessar o terminal bash interativo do contêiner de banco de dados para consultas SQL manuais:</strong><br/><code style="background-color: #1F2937; color: #F8FAFC; padding: 2px 6px; border-radius: 4px;">docker compose exec database psql -U postgres -d sistema_os_db</code></li>
</ul>
